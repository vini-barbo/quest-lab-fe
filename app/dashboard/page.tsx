    ...
    <div className="container space-y-6 p-4 py-6 md:p-8">
      <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
        <div>
          ...
          <p className="text-muted-foreground">Bem-vindo, {userName}! Aqui está o desempenho dos seus alunos.</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Questões Criadas</CardTitle>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">+42 na última semana</p>
          </CardContent>
        </Card>
      </div>
            ...
            {mockStudentData.map((student) => (
              <Card key={student.id}>
                <CardHeader className="pb-2">
                  ...
                  <CardDescription>{student.questionsAnswered} questões respondidas</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium">
                      Taxa de acerto: {Math.round((student.correctAnswers / student.questionsAnswered) * 100)}%
                    </span>
                    <span className="text-sm text-muted-foreground">
                      {student.correctAnswers}/{student.questionsAnswered}
                    </span>
                  </div>
                  <Progress value={(student.correctAnswers / student.questionsAnswered) * 100} className="mt-2" />
                </CardContent>
              </Card>
            ))}
          </div>
            ...
            {mockCategoryData.map((category) => (
              <Card key={category.id}>
                <CardHeader className="pb-2">
                  ...
                  <CardDescription>{category.questionsCount} questões</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium">Média de acertos: {category.averageScore}%</span>
                  </div>
